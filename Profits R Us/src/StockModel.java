...
public class StockModel {
	...
	private XYChart.Series<String, Number> stockPriceSeries  = new XYChart.Series<String, Number>();
	
	public void createStockPriceSeries(int timeline){
		
		String tempDailyStockData;
		String[] columnDataArray;
        LinkedList<DatePriceObject> allStockValues = new LinkedList<DatePriceObject>();
        LinkedList<DatePriceObject> smoothStockValues = new LinkedList<DatePriceObject>();
        ...
        try{
			...
			if(timeline == 0){
				while(csvFile.hasNextLine()){
					...
					allStockValues.add(new DatePriceObject(columnDataArray[0], Double.parseDouble(columnDataArray[6])));
				}
			}
			else
        ...
        for (int i = allStockValues.size() ; i > 0 ; i--){
        	...
        	if(queueLineSmoothing.size()< dataPointDivider){
				...
				if(count%dataPointDivider == 0)
					smoothStockValues.add(new DatePriceObject(temp.getDate(), lineSmoothing/dataPointDivider));
